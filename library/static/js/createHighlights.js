/***
  When invoked, this function will create highlights, modals and all needed control elements to handle
  clicks on highlights, controlling audio playback and videos
  
  Properties:
    players - an array of Popcorn objects that is passed on to the function
      players = [ Popcorn("#audioId"), Popcorn("#audioId2") ...]
    
    instruments - an array of instrument objects 
      instruments = [ 
                      {
                        id: "instrumentId",
                        orderNumber: 0 // the order in which highlights are shown with the players audio playing
                        name: "Attitude Indicator (AI)",
                        image: "/static/attitudeIndicator_wBg.png",
                        audio: [ "/static/someInitAudio.mp3" ],
                        top : "5%",
                        left : "36.2%",
                        width : "26%",
                        height: "41%",
    ...
    highlights - an array of an array of highlights that will be shown on top of the desired instrument
      // in the same order as the instruments  
      highlights =  [
                      // highlights inside the modal for the first instrument
                      [ 
                        {
                          top : "35%",
                          left : "24%",
                          ...
                          audio: [ "//online.cdot.senecacollege.ca/c4x/Seneca_College/M01S01_Test/asset/placeholderTomAudio.mp3" ]
                        },
                        ....
                      ],

                      // highlights inside the modal for the second instrument
                      [
                        {
    ...
    buttons - an array of an array of buttons that will be displayed inside each modal
    // in the same order as the instruments
    *** reset button must be included (if you want one), with a blank 'video' property or
          'Back to Instrument' text
      buttons = [
                  [ // first instrument buttons
                    {
                      title: "Pitch Up",
                      video: "/static/attitudeUp.mp4",
                      relatedHighlight: 3,
                    },
                    {
                      title: "Pitch Down",
                      video: "/static/attitudeDown.mp4",
                      relatedHighlight: 3,
                    },
                    {
                      title: "Bank Left",
                      video: "/static/attitudeLeft.mp4",
                      relatedHighlight: 1,
                    },
                    {
                      title: "Bank Right",
                      video: "/static/attitudeRight.mp4",
                      relatedHighlight: 1,
                    },
                  ...
                  [ // second instrument butons
                    {
                      ...
                    }
                  ],

                  [], // third instrument buttons
                  ...
                ]
  **/
var createHighlights = function( instruments, highlights, buttons, options ){
  var md = new MobileDetect(window.navigator.userAgent), isMobile, hasListened = [], reset, players=[], btnAudioPlaying = false,
      hasClosed = [], buttonEnabled = [], active , manualCancelTimer, manualCancel, cancellingRedirect, invokerAction,
  ...
  pause = options.pause || $("#btnPause");
  play =  options.play || $("#btnPlay");
  replay = options.replay || $("#btnR");
  previous = options.previous || $("#btnP");
  next = options.next || $("#btnN");
  btnC = options.btnC || $("#btnC");
  btnB = options.btnB || $("#btnB");
  cancelRedirect = options.cancelRedirect || $("#cancelRedirect");
  modalDefaultMessage = options.modalDefaultMessage || "Select a highlight/button";
  defaultMessage = options.defaultMessage || "Select the highlighted instrument to continue";
  playStatus = options.playStatus || "Playing...";
  pauseStatus = options.pauseStatus|| "Paused";
  backId = options.backId || "aa53d0cd988547e084d69d59f4dbe7a3"; 
  continueId = options.continueId || "a01099b4746644e6b4bfcebeea88ab22";
  continueTime = options.continueTime || 10; // time until automatically continues
  highlightsDisable = options.highlightsDisable || false;
  closeModalButton = options.closeModalButton || "Back to Panel";
  allowEasyClose = options.allowEasyClose || false;
  modalNote = options.modalNote || "* all highlights and videos must be explored before this panel can be closed";
  altModalMessage = options.altModalMessage || "Continue when ready";
  noModal = options.noModal || false;
  reverseAudio = options.reverseAudio || false;
  statusClickMessage = options.statusClickMessage || "Click on the ";
  hideInvokerBorder = options.hideInvokerBorder || false;
  redirectId = options.redirectId || "1862e788b22443758a96795e226d34e9";
  showTitles = options.showTitles || false;
  snap3slide2special = options.snap3slide2special || false;
  ensureNoDblClick = options.ensureNoDblClick || false;
  resetImage = options.resetImage || false;
