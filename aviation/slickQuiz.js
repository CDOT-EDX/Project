...
function checkCorrectAnswer(quizId, questionIndex, selectedAnswer) {
    ...
    var optionsFetch = anySlide.options.quizzes;
    console.log("options length " + optionsFetch.length);

    //console.log("Here it is " + anySlide.options.quizzes[quizId].json.questions.length);

    console.log("Quiz# " + quizId);
    console.log("Questions# " + questionIndex);
    console.log("Selected Answer# " + selectedAnswer);

    console.log("Here it is :) " + JSON.stringify(anySlide.options.quizzes[quizId].json.questions[questionIndex].a[selectedAnswer]));

    return anySlide.options.quizzes[quizId].json.questions[questionIndex].a[selectedAnswer];
}
        ...
        plugin.method = {
                ...
                for (i in questions) {
                    if (questions.hasOwnProperty(i)) {
                        ...
                        if (plugin.config.displayQuestionCount) {
                            questionHTML.append('<div class="' + questionCountClass + '">' + plugin.config.questionCountText.replace('%current', '<span class="current">' + count + '</span>')
                                .replace('%total', '<span class="total">' + questionCount + '</span>') + '</div>');
                        }
            ...
            checkAnswer: function (checkButton, options) {
                ...
                $(anySlide).on('checkCompleted', function(evt, data){
                    ...
                    if(storedAttempts >= attemptsBeforeRemediation && attemptsBeforeRemediation !== 0 && !correctResponse){
